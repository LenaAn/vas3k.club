...
from common.data.tags import HOBBIES, PERSONAL, WHERE, CLUB
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        update_tag_group(Tag.GROUP_WHERE, WHERE)
        self.stdout.write(f"{len(WHERE)} where")
        ...
        all_tag_keys = list(dict(HOBBIES).keys()) \
            ...
            + list(dict(WHERE).keys()) \
            + list(dict(CLUB).keys())
        Tag.objects\
            .filter(group__in=[Tag.GROUP_HOBBIES, Tag.GROUP_PERSONAL, Tag.GROUP_WHERE, Tag.GROUP_CLUB])\
